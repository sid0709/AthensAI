...
import { safeRuntimeSendMessage } from "./runtimeMessaging";
...
export const messageHandler = (request, sender, sendResponse) => {
			...
			switch (request.action) {
				...
				case 'swanFetch': {
					...
					try {
						...
						if (payload.body != null && method !== 'GET' && method !== 'HEAD') {
							...
							init.body = typeof payload.body === 'string'
								? payload.body
						...
						sendResponse?.({ success: true, status, data, ok: resp.ok });
					} catch (err) {
				...
				case 'executeAction': {
					try {
						...
						if (action === 'fetch') {
							const waitMs = typeof identifier === 'string' && identifier.includes('wait_for') ? 8000 : 3000;
							// Resolve target element(s)
							let elements = findElements(componentType, propertyName, pattern);
							if (!elements || elements.length === 0) {
								elements = await waitForElements(componentType, propertyName, pattern, waitMs, 100);
							}
							if (!elements || elements.length === 0) {
								safeRuntimeSendMessage({ action: 'fetchResult', payload: { identifier, success: false, error: 'No elements found' } });
								break;
							}
							...
							safeRuntimeSendMessage({ action: 'fetchResult', payload: { identifier, success: true, data } });
						} else {
						...
						const payload = request.payload || {};
						if (payload.action === 'fetch' && payload.identifier) {
	...
	return request.action === 'EXTRACT_MAIN_CONTENT'
		|| request.action === 'FIND_INTERACTABLE_ELEMENTS'
		|| request.action === 'scraper:ping'
		|| request.action === 'swanFetch';
};
