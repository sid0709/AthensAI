...
| `.athens-segment` | Grouped destinations sized to their labels, not equal-width columns |
...
| `.athens-status` | Status pill with a Lucide icon plus label |
...
- Job status uses Lucide icons (inbox, clipboard, layers, check, calendar, x) rather than colored dots. Labels and selected fill must also communicate state.
- Loading, empty, and error states use concise language and the same neutral surface hierarchy.

## Typography and icons

- Use San Francisco (SF Pro) via `-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif` on opted-in surfaces. Do not load additional display fonts for chrome.
