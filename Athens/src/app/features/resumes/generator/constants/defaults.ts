...
function normalizeStoredSteps(steps: GenStep[]): GenStep[] {
...
/** Merge historical or canonical v4 persistence onto runtime defaults. */
export function mergeStoredConfig(parsed: unknown): GeneratorConfig {
  ...
  return ensurePurposes({
    ...
    systemInstruction: typeof raw.systemInstruction === "string"
    ...
    steps: savedSteps.length
      ? normalizeStoredSteps(savedSteps as GenStep[])
      : base.steps,
    coverage: normalizeCoverageSettings(raw.coverage),
  });
}

/** The reusable v4 config intentionally excludes transient ApplicationRun data. */
...
export function defaultPromptFor(_purpose: Purpose, _kind: StepKind): string {
  return "";
}
