...
test("scroll navigation advances one scene and stays within bounds", () => {
  assert.equal(nextAuthScene(0, 120, 3), 1);
  assert.equal(nextAuthScene(1, -120, 3), 0);
  assert.equal(nextAuthScene(1, 0, 3), 1);
  assert.equal(nextAuthScene(2, 120, 3), 2);
  assert.equal(nextAuthScene(0, -120, 3), 0);
});
