...
import { LiveMetricsPanel, type LiveMetricPoint, type LiveRange, type VpsAvailabilityPoint } from "./LiveMetricsPanel";
...
function AvailabilityHistory({ component, dates, rollups }: { component: ComponentStatus; dates: string[]; rollups: Map<string, Rollup> }) {
  const today = dates.at(-1) || "";
  return (
    <div className="mt-5">
      <div className="flex items-center justify-between text-[11px] font-medium text-slate-500"><span>90 days ago</span><span>Today</span></div>
      <div className="mt-2 grid h-9 grid-cols-[repeat(90,minmax(2px,1fr))] gap-[2px]" aria-label={`${component.name} 90-day availability`}>
        {dates.map((date) => {
          ...
          return <span key={date} role="img" aria-label={description} title={description} className={`min-w-0 rounded-[2px] ${historyStyle[state].className}`} style={historyStyle[state].style} />;
        })}
      </div>
    </div>
  );
  ...
  return (
    <div className="status-page-scroll h-full overflow-y-auto bg-[#f8fafc] text-slate-950">
      ...
      <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:py-10">
        ...
        <LiveMetricsPanel points={livePoints} range={liveRange} onRangeChange={setLiveRange} updatedAt={liveUpdatedAt} loading={liveLoading} error={liveError} vpsStatus={vpsStatus} availability={vpsAvailability} />
            ...
            {serviceComponents.map((component) => {
              ...
              return (
                <article key={component.component} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] transition-shadow hover:shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-6">
                  ...
                  <AvailabilityHistory component={component} dates={historyDates} rollups={historyByComponent} />
                </article>
              );
            })}
          </div>
