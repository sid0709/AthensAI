...
Prometheus is the source of current and time-series status data. **athens-backend** writes compact MongoDB snapshots (`monitor_status`, `monitor_days`, `monitor_incidents`) and exposes application metrics on private port `9101` (reachable through the `athens-monitoring` Docker network).

## Credentials

No Google Cloud Monitoring credential is used. Prometheus only collects VPS-local, container, and application health signals. Durable status rows live in MongoDB (`AthensDB`); Firebase is used only for Storage (e.g. bid recordings). No credential belongs in Git.
...
Daily Mongo summaries are written only after a closed UTC day has at least 95 percent Prometheus coverage for every public component.
